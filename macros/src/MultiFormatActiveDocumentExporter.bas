Attribute VB_Name = "MultiFormatActiveDocumentExporter"
Option Explicit

' Multi-Format Active Document Exporter
...
' Assumptions: Open saved SOLIDWORKS document
...
Sub main()
    ...
    Exit Sub
Fail:
    MsgBox "Macro failed: " & Err.Description, vbCritical, "Multi-Format Active Document Exporter"
End Sub
...
Private Sub RunMacro()
    ...
    stem = IIf("multi-format-active-document-exporter" = "export-with-custom-property-filename", PropertyFileStem(model, modelPath), CleanFileName(BaseName(modelPath)))
