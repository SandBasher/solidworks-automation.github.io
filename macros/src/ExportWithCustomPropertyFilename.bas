Attribute VB_Name = "ExportWithCustomPropertyFilename"
Option Explicit
...
' Assumptions: Open saved SOLIDWORKS document; Optional PartNo, Revision and Description custom properties
...
Sub main()
    ...
    Exit Sub
Fail:
    MsgBox "Macro failed: " & Err.Description, vbCritical, "Export PDF/DXF/STEP with Filename from Custom Properties"
End Sub
...
Private Sub RunMacro()
    ...
    outFolder = CombinePath(FolderOf(modelPath), "Exports")
    EnsureFolder outFolder
    stem = IIf("export-with-custom-property-filename" = "export-with-custom-property-filename", PropertyFileStem(model, modelPath), CleanFileName(BaseName(modelPath)))
    If model.GetType = swDocDRAWING Then
        model.Extension.SaveAs CombinePath(outFolder, stem & ".pdf"), 0, swSaveAsOptions_Silent, Nothing, errors, warnings
        model.Extension.SaveAs CombinePath(outFolder, stem & ".dxf"), 0, swSaveAsOptions_Silent, Nothing, errors, warnings
        model.Extension.SaveAs CombinePath(outFolder, stem & ".dwg"), 0, swSaveAsOptions_Silent, Nothing, errors, warnings
    ElseIf model.GetType = swDocPART Or model.GetType = swDocASSEMBLY Then
        model.Extension.SaveAs CombinePath(outFolder, stem & ".step"), 0, swSaveAsOptions_Silent, Nothing, errors, warnings
        model.Extension.SaveAs CombinePath(outFolder, stem & ".stl"), 0, swSaveAsOptions_Silent, Nothing, errors, warnings
    Else
        Err.Raise vbObjectError + 301, , "Unsupported document type."
    End If
    MsgBox "Exports written to " & outFolder, vbInformation, "Export complete"
End Sub
