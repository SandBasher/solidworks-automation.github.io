Attribute VB_Name = "ExportDrawingSheetsSeparatePdf"
Option Explicit
...
Sub main()
    ...
    Exit Sub
Fail:
    MsgBox "Macro failed: " & Err.Description, vbCritical, "Export Each Drawing Sheet as a Separate PDF"
End Sub
...
Private Sub RunMacro()
    ...
    If drawing.GetType <> swDocDRAWING Then Err.Raise vbObjectError + 700, , "Open a drawing."
    drawingPath = EnsureSaved(drawing)
    outFolder = CombinePath(FolderOf(drawingPath), "SheetPDF")
    EnsureFolder outFolder
    sheetNames = drawing.GetSheetNames
    For Each sheetName In sheetNames
        drawing.ActivateSheet CStr(sheetName)
        drawing.Extension.SaveAs CombinePath(outFolder, CleanFileName(BaseName(drawingPath) & "_" & CStr(sheetName)) & ".pdf"), 0, swSaveAsOptions_Silent, Nothing, errors, warnings
        count = count + 1
    Next sheetName
    MsgBox "Exported " & count & " sheet PDFs to " & outFolder, vbInformation, "Sheet export complete"
End Sub
